    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If IsPostBack Then
                ...
                '***********************************************************************
                'Added by Pragnesha Kulkarni on 2019/05/16
                'Purspose:resolve an error of exam result 
                'Desc:Load an image till complete te end exam process
                '***********************************************************************
                Dim script As String = "$(document).ready(function () { $('[id*=imgBtnEndExam]').click(); });"
            ...
            If objconn.connect() Then
                ...
                For iLoop = intStart To intEndNo
                    ...
                    ElseIf objStructureQuePaper.QuesTypeID = "2" Then
                        '***********************************************************************
                        'Added by Pragnesha Kulkarni on 2018/06/12
                        'Purspose: Allow user to choose single option 
                        'Desc:Checkbox code replaced with radio button
                        '***********************************************************************
                        'Code for radio button
                        For jLoop = 0 To optcount - 1
                            'Create Option Section
                            ...
                            i = i + 1
                            ReDim Preserve rdbut(UBound(rdbut) + 1)
                            'Ended by Pragnesha Kulkarni on 2018/06/12
                            '***********************************************************************
                            ...
                            'commented by Pragnesha Kulkarni and changed by radio button for single option selection on 2018/06/12
                            'Purpose:Avoid multiple selection of option
                            ''******* To get the keys in array to traverse later **************
                            'Dim keys(objStructureQuePaper.Options.Count) As Integer
                            'objStructureQuePaper.Options.Keys.CopyTo(keys, 0)
                            'Array.Reverse(keys)
                            ...
                            'Comment Ended by Pragnesha Kulkarni on 2018/06/12
                ...
                Try
                    If objconn.connect() = True Then
                        ...
                        For arrLoop = 0 To strCheckedArray.Length - 2
                            ...
                            '  mCmd = New SqlCommand(qr, objconn.MyConnection, transaction)
                            '  mda = New SqlDataAdapter(mCmd)
                            ...
                            If intQuestype = 1 Then
                                ...
                                Try
                                    If dss.Tables(0).Rows.Count > 0 Then
                                        ...
                                        transaction = objconn.MyConnection.BeginTransaction()
                                        ...
                                        '-----------------------------------------------
                                        'Desc: update T_result_option one by one 
                                        'Bug ID:1183,1188
                                        'Changed by: Pragnesha Date: 25-7-2019
                                        '----------------------------------------------
                                        mcommand = New SqlCommand("Update T_result_option with(rowlock) set option_id=" & optionNo & "where result_id = " & dss.Tables(0).Rows(0).Item(0).ToString, objconn.MyConnection, transaction)
                                        mcommand.ExecuteNonQuery()
                                        transaction.Commit()
                                        '-------------------------------------------
                                        ' ViewState.Add("strqestionSql", ViewState("strqestionSql") + update)  ' Commented for adding record one by one
                            ...
                            ElseIf intQuestype = 2 Then
                                ...
                                If (arrLoop = 0) Then
                                    ...
                                    mcommand.ExecuteNonQuery()
                                    mcommand = Nothing
                                    'ViewState.Add("strqestionSql", ViewState("strqestionSql") + dqry) 'Commented for Delete recordID Repeat one by one
                                End If

                                Try
                                    transaction = objconn.MyConnection.BeginTransaction()
                                    Dim qry As String = "Insert into T_result_option(result_id,option_id) Values(" & dss.Tables(0).Rows(0).Item(0).ToString & "," & optionNo & ")" & ";"
                                    mcommand = New SqlCommand(qry, objconn.MyConnection, transaction)
                                    mcommand.ExecuteNonQuery()
                                    transaction.Commit()
                                    ' ViewState.Add("strqestionSql", ViewState("strqestionSql") + qry) ' Commented for adding record one by one
                                Catch ex As Exception
                                    transaction.Rollback()
                                    If log.IsDebugEnabled Then
                                        log.Debug("Error :" & ex.ToString())
    ...
    Function UpdateUserstatusAfterExam()
        ...
        Try
            If objconn.connect() = True Then
                ...
                strSqlBuilder.Append("Update t_candidate_status set appearedflag = '2',written_test_appear_date = '" & Now & "',MailSent_flag='1'")
                ...
                '-------------------------------
                'reopened for Update result one by one
                'Bug ID:1183,1188
                'Changed by : Pragnesha Date: 25-7-2019
                '--------------------------------------
                myCommand = New SqlCommand(strSqlBuilder.ToString(), objconn.MyConnection, transaction)
                myCommand.ExecuteNonQuery()
                transaction.Commit()
                '-------------------------------------------------------
...
#Region "Submit form with finish button"
    ...
    Function FormSubmitFinish()
        Try
            ...
            '=================
            'dipendra 'Commented for single record save
...
#End Region
            ...
            For Each item In htFinal
                ...
                For i As Integer = 1 To 3
                    ...
                    For j As Integer = 0 To 1
                        ...
                        'Commented by Nisha on 2018/05/08
                        'Reason: When student appearing combine test system error is occured 
                        ...
                        'Ended by Nisha on 2018/05/08

                        'Modified by Nisha on 2018/05/08
                        ...
                        'Added by bhumi [1/9/2015]
                        'Reason: For MNNV L5 incorrect Test_Type getting so at exam time error occured
                        stbldr.Append(" where del_flag=0 AND TEST_TYPE IN (SELECT test_type from M_Weightage where Course_ID=" & Cid & "and test_type=" & testtype & ") and Qn_Category_ID=" & i.ToString & " and qlevel=" & j.ToString)
                        'ended by bhumi
                        stbldr.Append(" group by  test_type,Qn_Category_ID,total_marks,qlevel,qnid ")
                        stbldr.Append(" order by Qn_Category_ID,qlevel,newid()")
                        'Ended by Nisha on 2018/05/08
    ...
    Public Function GetCandidateResult(ByVal CandidateID As String, ByVal courseID As String) As Hashtable
        ...
        strBldr.Append("  tcs.written_test_appear_date as appearancedate,  tcs.appearedflag as appearedflag, tcs.MailSent_flag as MailSentFlag,  mc.course_name,mce.Center_Name,  ")
        ...
        Try
            If objconn.connect() = True Then
                ...
                For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                    ...
                    ht.Add("Centrename", ds.Tables(0).Rows(0).Item(8).ToString)
                    ht.Add("Coursename", ds.Tables(0).Rows(0).Item(7).ToString)
                    ht.Add("Date", Date.Today.ToString("dd/MM/yyyy"))
                    ht.Add("TotalMark", ds.Tables(0).Rows(0).Item(9).ToString)
                    ht.Add("MarksObtained", ds.Tables(0).Rows(0).Item(11).ToString)
                    ht.Add("Status", ds.Tables(0).Rows(0).Item(13).ToString)
                Next
                ...
                stransaction = objconn.MyConnection.BeginTransaction
                For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                    strquery = "Insert into Final_Result([UserID] ,[UserName],[CourseCode],[CourseID],[CourseName],[Appear_Flag],[MailSent_flag],[Appear_Date],[CenterName],[TotalMarks],[TotalPassMarks],[Obtained_Marks],[Grade],[Exam_status]) values('" & ds.Tables(0).Rows(0).Item(0).ToString & "','" & ds.Tables(0).Rows(0).Item(1).ToString & "','" & ds.Tables(0).Rows(0).Item(2).ToString & "','" & ds.Tables(0).Rows(0).Item(3).ToString & "','" & ds.Tables(0).Rows(0).Item(7).ToString & "','" & ds.Tables(0).Rows(0).Item(5).ToString & "','" & ds.Tables(0).Rows(0).Item(6).ToString & "','" & Date.Today.ToString("yyyyMMdd") & "','" & ds.Tables(0).Rows(0).Item(8).ToString & "','" & ds.Tables(0).Rows(0).Item(9).ToString & "','" & ds.Tables(0).Rows(0).Item(10).ToString & "','" & ds.Tables(0).Rows(0).Item(11).ToString & "','" & ds.Tables(0).Rows(0).Item(12).ToString & "','" & ds.Tables(0).Rows(0).Item(13).ToString & "')"
                Next

                myCommand = New SqlCommand(strquery, objconn.MyConnection, stransaction)
                myCommand.ExecuteNonQuery()
                stransaction.Commit()
                '------------------------START-----------------------------
                'Added By: Bharat Prajapati
